...
function taskDesignRequest(comments: IssueComment[], issueNumber: number): TaskDesignRequest {
  ...
  if (requestIndex < 0 || requestIndex < latestCompleted) return { active: false };
...
export async function processNeedsDesignTask(
  ...
  let claimed = false;
  try {
    const initialRequest = taskDesignRequest(await ops.listComments(card), card.number);
    ...
    if (!isNeedsDesignTask(fresh, cfg) || !ownsTaskDesignClaim(fresh, botLogin)) return "skipped";
    ...
    if (design.openQuestions.length > 0) {
      ...
      callback(`Task "${latest.title}" remains in ${cfg.columns.needs_design}: ${design.openQuestions.length} open question(s).`, "warn");
      return "questioned";
    }
...
export class BoardLoop {
  ...
  private async processTaskDesignCards(cards: Card[]): Promise<void> {
    ...
    const baseOps = this.deps.taskDesignOps ?? {
      ...
      refresh: (candidate: Card) => getCard(candidate.itemId, cfg.status_field, cfg.plan_field, cfg.type_field),
      ...
      setReady: (candidate: Card) => setStatus(meta, candidate.itemId, cfg.columns.ready),
