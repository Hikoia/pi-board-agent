...
export type ExecutorStatusCallback = (message: string, level?: "info" | "warn" | "error") => void;
...
export interface TicketWorkflowManager {
  start(
    ...
    options: { maxAgents: number; concurrency: number; agentRetries: number; agentTimeoutMs?: number },
  ): string;
...
function runArgsMatch(run: PersistedRunState, record: TicketWorktreeRecord): boolean {
  if (!run.args || typeof run.args !== "object") return false;
  const args = run.args as Record<string, unknown>;
  return args.itemId === record.itemId &&
...
export function createWorkflowManagerAdapter(options: {
  ...
  const onError = (event: any) => options.callback(
  ...
  const scheduler = new UsageLimitScheduler(manager, {
    onDiagnostic: (message) => options.callback(`Workflow scheduler: ${message}`, "warn"),
  });
  ...
  return {
    start(script, args, exec) {
      const started = manager.startInBackground(script, args, {
        ...
        ...(exec.agentTimeoutMs === undefined ? {} : { agentTimeoutMs: exec.agentTimeoutMs }),
      });
...
export class ManagedTicketExecutor implements TicketExecutor {
  ...
  private async commentOnce(card: Card, uniqueMarker: string, body: string): Promise<void> {
  ...
  private async moveToNeedsHuman(
    ...
    await this.deps.board.release(card);
    if (record.schemaVersion === 2) this.deps.worktrees.clearExecution(record.itemId, runId);
    this.deps.callback(`"${card.title}" → ${this.deps.cfg.columns.needs_human}: ${reason}`, "warn");
  }
  ...
  private async complete(
    ...
    if (!statusIs(card, this.deps.cfg.columns.review)) {
      await this.deps.board.setStatus(card.itemId, this.deps.cfg.columns.review);
      card.status = this.deps.cfg.columns.review;
    }
    await this.deps.board.release(card);
    this.deps.worktrees.clearExecution(record.itemId, runId);
    this.deps.callback(`"${card.title}" succeeded → ${this.deps.cfg.columns.review}. Worktree: ${record.path}`);
  }
  ...
  private matchingLaunchRuns(record: TicketExecutionRecord, manager: TicketWorkflowManager): PersistedRunState[] {
    ...
    return manager.list().filter((run) =>
  ...
  private async recoverLaunching(
    ...
    try {
      ...
      await this.moveToNeedsHuman(record, card, `cannot open persisted workflow state: ${error.message}`);
      summary.needsHuman++;
      return undefined;
    }
    ...
    if (matches.length === 1) {
      const run = matches[0];
      const adopted = this.deps.worktrees.setActiveRun(record.itemId, run.runId, new Date(run.startedAt).getTime());
      summary.adopted++;
      this.deps.callback(`Adopted persisted run ${run.runId} for "${card.title}".`);
      return adopted;
    }
    if (matches.length > 1) {
      await this.moveToNeedsHuman(record, card, "multiple persisted runs match the interrupted launch");
      summary.needsHuman++;
      return undefined;
    }
    ...
    if (check.ok && !this.deps.worktrees.hasTaskDelta(record)) {
      if (statusIs(card, this.deps.cfg.columns.building)) {
        await this.deps.board.setStatus(card.itemId, this.deps.cfg.columns.ready);
        card.status = this.deps.cfg.columns.ready;
      }
      await this.deps.board.release(card);
      this.deps.worktrees.clearExecution(record.itemId);
      this.deps.callback(`Interrupted launch for "${card.title}" had no side effects; returned to ${this.deps.cfg.columns.ready}.`, "warn");
      return undefined;
    }

    await this.moveToNeedsHuman(record, card, check.reason ?? "task branch changed before a run could be identified");
    summary.needsHuman++;
    return undefined;
  }
  ...
  private async stopForManualState(
    ...
    if (statusIs(card, this.deps.cfg.columns.ready)) {
      const check = this.deps.worktrees.check(record, true);
      if (!check.ok) {
        await this.moveToNeedsHuman(record, card, `manual retry is unsafe: ${check.reason}`, record.activeRunId, "manual-dirty");
        return;
      }
    }
    await this.deps.board.release(card);
    this.deps.worktrees.clearExecution(record.itemId, record.activeRunId);
    this.deps.callback(`Stopped stale run ${record.activeRunId}; preserved manual status ${card.status ?? "unknown"}.`, "warn");
  }
  ...
  private async reconcileActive(
    ...
    try {
      ...
      await this.moveToNeedsHuman(record, card, `cannot open workflow manager: ${error.message}`, record.activeRunId, "manager-error");
      summary.needsHuman++;
      return;
    }

    const run = manager.list().find((candidate) => candidate.runId === record.activeRunId);
    if (!run || !runArgsMatch(run, record)) {
      await this.moveToNeedsHuman(record, card, run ? "workflow arguments do not match the ticket record" : "persisted workflow run is missing", record.activeRunId, "missing");
      summary.needsHuman++;
      return;
    }
    ...
    if (!structural.ok || !planMatches || (record.issueNumber > 0 && card.number !== record.issueNumber)) {
    ...
    if (run.status === "paused") {
      ...
      if (!check.ok) {
        await manager.stopAndWait(run.runId);
        await this.moveToNeedsHuman(record, card, check.reason ?? "paused worktree is unsafe", run.runId, "dirty");
        summary.needsHuman++;
        return;
      }
      ...
      if (await manager.resume(run.runId)) {
        summary.resumed++;
        summary.active.push({ itemId: record.itemId, taskKey: record.taskKey, runId: run.runId, status: "running", worktree: record.path });
        this.deps.callback(`Resumed ${run.runId} for "${card.title}".`);
        return;
      }
      const raced = manager.list().find((candidate) => candidate.runId === run.runId);
      if (raced?.status === "running") {
        summary.active.push({ itemId: record.itemId, taskKey: record.taskKey, runId: run.runId, status: "running", worktree: record.path });
        return;
      }
      await this.moveToNeedsHuman(record, card, "paused workflow could not be resumed", run.runId, "resume-failed");
      summary.needsHuman++;
      return;
    }
    ...
    if (run.status === "completed") {
      ...
      const outcome = outcomes.length === 1 && outcomes[0].itemId === record.itemId && outcomes[0].taskKey === record.taskKey
        ? outcomes[0]
        : undefined;
      const check = this.deps.worktrees.check(record, true);
      if (!outcome || !check.ok || (outcome.status === "success" && outcome.branch !== record.taskBranch)) {
        await this.moveToNeedsHuman(
          record,
          card,
          !check.ok ? check.reason ?? "completed worktree is unsafe" : "persisted builder result is malformed",
          run.runId,
          "malformed",
        );
        summary.needsHuman++;
    ...
    await this.moveToNeedsHuman(record, card, run.error ?? `workflow ended as ${run.status}`, run.runId, run.status);
    summary.needsHuman++;
  }

  private async stopMissingCardRun(record: TicketExecutionRecord, summary: ReconcileSummary): Promise<void> {
    ...
    summary.orphans++;
    this.deps.callback(`Stopped orphaned ticket run ${record.activeRunId ?? record.itemId}; its Project item is gone and worktree was preserved.`, "warn");
  }

  async reconcile(cards: Card[]): Promise<ReconcileSummary> {
    const summary: ReconcileSummary = {
      active: [], resumed: 0, adopted: 0, needsHuman: 0, legacy: 0, orphans: 0, errors: 0,
    };
    ...
    for (const legacy of this.deps.legacyInflight.list()) {
      ...
      if (!card) {
        summary.errors++;
        this.deps.callback(`Legacy inflight record has no board card: ${legacy.itemId}`, "warn");
        continue;
      }
      try {
        const fresh = await this.deps.board.getCard(card.itemId) ?? card;
        const uniqueMarker = `<!-- board-agent-legacy-inflight:${legacy.itemId} -->`;
        await this.commentOnce(fresh, uniqueMarker, "⚠️ A pre-persistence builder lock was found. The ticket was quarantined instead of retried.");
        await this.deps.board.setStatus(fresh.itemId, this.deps.cfg.columns.needs_human);
        ...
        summary.legacy++;
        summary.needsHuman++;
        this.deps.callback(`Archived legacy inflight state for "${fresh.title}" → ${this.deps.cfg.columns.needs_human}.`, "warn");
      } catch (error: any) {
        summary.errors++;
        this.deps.callback(`Legacy recovery failed for ${legacy.itemId}: ${error.message}`, "warn");
      }
    }
    ...
    for (const original of records) {
      ...
      if (!snapshot) {
        try {
          ...
          summary.errors++;
          this.deps.callback(`Could not verify missing board card ${original.itemId}: ${error.message}`, "warn");
          continue;
        }
        if (!snapshot) {
          if (original.schemaVersion === 2 && (original.activeRunId || original.launchingAt)) {
            try {
              await this.stopMissingCardRun(original, summary);
            } catch (error: any) {
              summary.errors++;
              this.deps.callback(`Could not stop orphaned ticket ${original.itemId}: ${error.message}`, "warn");
            }
          }
          continue;
        }
      ...
      try {
        const card = await this.deps.board.getCard(original.itemId) ?? snapshot;
        ...
        if (original.schemaVersion !== 2) {
          if (statusIs(card, this.deps.cfg.columns.building)) {
            await this.moveToNeedsHuman(original, card, "legacy worktree record has no managed workflow run");
            summary.needsHuman++;
          }
          continue;
        }
        ...
        if (record.launchingAt && !record.activeRunId) record = await this.recoverLaunching(record, card, summary);
        if (record?.activeRunId) await this.reconcileActive(record, card, summary);
          ...
          await this.moveToNeedsHuman(record, card, "In Progress ticket has no active workflow run");
          summary.needsHuman++;
        }
      } catch (error: any) {
        summary.errors++;
    ...
    for (const snapshot of cards) {
      ...
      try {
        const card = await this.deps.board.getCard(snapshot.itemId) ?? snapshot;
        if (!statusIs(card, this.deps.cfg.columns.building)) continue;
        const uniqueMarker = `<!-- board-agent-orphan:${card.itemId} -->`;
        await this.commentOnce(card, uniqueMarker, "⚠️ This ticket is In Progress but has no execution record. It was quarantined without retrying.");
        await this.deps.board.setStatus(card.itemId, this.deps.cfg.columns.needs_human);
        card.status = this.deps.cfg.columns.needs_human;
        snapshot.status = card.status;
        await this.deps.board.release(card);
        summary.orphans++;
        summary.needsHuman++;
        this.deps.callback(`Orphaned ticket "${card.title}" → ${this.deps.cfg.columns.needs_human}.`, "warn");
      } catch (error: any) {
        summary.errors++;
        this.deps.callback(`Orphan recovery failed for ${snapshot.itemId}: ${error.message}`, "warn");
      }
    }
  ...
  private eligible(card: Card, expectedPlan: string, requireClaim = false): string | undefined {
    ...
    if (!statusIs(card, this.deps.cfg.columns.ready)) return `status is ${card.status ?? "unset"}`;
    if (!card.plan || planSlug(card.plan) !== expectedPlan) return "Plan changed";
    if (card.assignees.some((assignee) => assignee !== this.deps.botLogin)) return "another assignee is present";
    if (requireClaim && !card.assignees.includes(this.deps.botLogin)) return "claim was not retained";
    const record = this.deps.worktrees.read(card.itemId);
    if (record?.schemaVersion === 2 && (record.activeRunId || record.launchingAt)) return "ticket already has an active run";
    return undefined;
  }

  private async resetUnstarted(record: TicketExecutionRecord, card: Card, reason: string): Promise<LaunchResult> {
    const check = this.deps.worktrees.check(record, true);
    if (check.ok && !this.deps.worktrees.hasTaskDelta(record)) {
      if (!statusIs(card, this.deps.cfg.columns.ready)) {
        await this.deps.board.setStatus(card.itemId, this.deps.cfg.columns.ready);
      ...
      await this.deps.board.release(card);
      this.deps.worktrees.clearExecution(record.itemId);
      this.deps.callback(`Launch preparation failed for "${card.title}": ${reason}. Returned to ${this.deps.cfg.columns.ready}.`, "warn");
  ...
  async launch(snapshot: Card, expectedPlan: string): Promise<LaunchResult> {
    ...
    try {
      ...
      await this.deps.board.release(card);
      return { status: "skipped", reason: `plan branch preparation failed: ${error.message}` };
    }
    ...
    try {
      ...
      return this.quarantineWithoutRecord(card, `worktree preparation failed: ${error.message}`);
    }
    const check = this.deps.worktrees.check(record, true);
    if (!check.ok) {
      await this.moveToNeedsHuman(record, card, check.reason ?? "worktree is unsafe");
      return { status: "needs-human", reason: check.reason ?? "worktree is unsafe" };
    }
    ...
    try {
      ...
      await this.deps.board.release(card);
      this.deps.worktrees.clearExecution(record.itemId);
      return { status: "skipped", reason: `could not move ticket to ${this.deps.cfg.columns.building}: ${error.message}` };
    }
    ...
    try {
      ...
      return this.resetUnstarted(record, card, `workflow manager failed: ${error.message}`);
    }
    let runId: string;
    try {
      runId = manager.start(
        script,
        { itemId: record.itemId, issueNumber: record.issueNumber, taskKey: record.taskKey },
        {
          maxAgents: 1,
          concurrency: 1,
      ...
      if (matches.length === 1) {
        ...
        return this.resetUnstarted(record, card, `workflow launch failed: ${error.message}`);
      }
    }
    ...
    try {
      ...
      this.deps.callback(`Run ${runId} persisted but its ticket record was not updated: ${error.message}`, "warn");
    }
  ...
  activeCount(): number {
    ...
    for (const record of this.deps.worktrees.list()) {
      ...
      try {
        const run = this.manager(record.path).list().find((candidate) => candidate.runId === record.activeRunId);
        if (!run || run.status === "pending" || run.status === "running" || run.status === "paused") count++;
      } catch {
        count++;
      }
    }
  ...
  async shutdown(): Promise<void> {
    ...
    this.managers.clear();
    if (errors.length > 0) throw new Error(`Could not pause workflow run(s): ${errors.join("; ")}`);
  }
}
...
export function createProductionTicketExecutor(options: {
  ...
  const board: TicketBoardAdapter = {
    getCard: (itemId) => getCard(itemId, options.cfg.status_field, options.cfg.plan_field, options.cfg.type_field),
    ...
    async listComments(card) {
      ...
      return (await listIssueComments(card.repoOwner, card.repoName, card.number)).map((comment) => comment.body);
    },
    async comment(card, body) {
      if (!card.number || !card.repoOwner || !card.repoName) throw new Error("Ticket is not backed by a GitHub issue");
      const issueId = await resolveIssueId(card.repoOwner, card.repoName, card.number);
  ...
  return new ManagedTicketExecutor({
    ...
    createManager: (cwd) => createWorkflowManagerAdapter({
      cwd,
      modelRegistry: options.modelRegistry,
      mainModel: options.mainModel,
      sessionId: options.sessionId,
    ...
    ensurePlan: (branch) => ensurePlanBranch(branch, options.cfg.branches.base, options.cwd),
...
export function inspectTicketExecutions(
  ...
  cards: Card[],
  cfg: Config,
): { active: ActiveTicketRun[]; legacy: number; orphans: number; needsHuman: number } {
  ...
  const active = records.flatMap((record): ActiveTicketRun[] => {
    ...
    let status = "missing";
    try {
      status = createRunPersistence(record.path).load(record.activeRunId)?.status ?? "missing";
    } catch {
      status = "unreadable";
    }
    return [{
  ...
  for (const card of cards) {
    if (card.type?.toLowerCase() !== "story" && statusIs(card, cfg.columns.building) && !recordIds.has(card.itemId)) orphanIds.add(card.itemId);
  }
  for (const record of records) if (!cardIds.has(record.itemId)) orphanIds.add(record.itemId);
  for (const run of active) if (run.status === "missing" || run.status === "unreadable") orphanIds.add(run.itemId);
  return {
    active,
    legacy: new Inflight(cwd).list().length,
    orphans: orphanIds.size,
    needsHuman: cards.filter((card) => statusIs(card, cfg.columns.needs_human)).length,
  };
}
