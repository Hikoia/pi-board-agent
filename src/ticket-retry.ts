...
export function pendingTicketWrite(record: TicketExecutionRecord): PendingWrite | undefined {
  ...
  const value = JSON.parse(record.retry.reason.slice(PREFIX.length)) as PendingWrite;
  if (!value?.card || value.card.itemId !== record.itemId || value.card.number !== record.issueNumber ||
      !isTargetIssue(value.card, value.card.repoOwner!, value.card.repoName!, "Task") ||
...
export async function settleTicketWrite(
  ...
  const target = (card: Card | undefined): card is Card => !!card &&
    isTargetIssue(card, write.card.repoOwner!, write.card.repoName!, "Task") &&
  ...
  card = await guard();
  if (!card) return "withdrawn";
  if (hasBot(card)) await board.release(card);
  sameRecord();
  finish();
  return "settled";
}
