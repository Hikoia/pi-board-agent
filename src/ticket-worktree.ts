...
export type TicketWorktreeRecord =
  | TicketExecutionRecord
  | LegacyTicketWorktreeRecord;
...
function branchExists(branch: string, cwd: string): boolean {
  return git(["rev-parse", "--verify", "--quiet", `refs/heads/${branch}`], cwd)
    .ok;
}
...
function isRecord(value: unknown): value is TicketWorktreeRecord {
  ...
  return (
    ["itemId", "taskKey", "plan", "taskBranch", "planBranch", "path"].every(
...
export class TicketWorktrees {
  ...
  update(
    ...
    if (!current || current.schemaVersion !== 2) {
      throw new Error(
        `Ticket execution record is missing or legacy: ${itemId}`,
      );
    }
  ...
  isMerged(itemId: string, targetBranch: string, taskBranch?: string): boolean {
    ...
    return (
      marked ||
      (!!taskBranch &&
        this.isIntegrated(remoteTarget, `origin/${taskBranch}`, this.repoRoot))
    );
  }
  ...
  private isIntegrated(
    ...
    if (git(["merge-base", "--is-ancestor", taskRef, targetRef], cwd).ok)
      return true;
    const cherry = git(["cherry", targetRef, taskRef], cwd);
  ...
  check(record: TicketWorktreeRecord, requireClean = true): WorktreeCheck {
    if (!this.isManagedPath(record.path))
      return {
        ok: false,
        clean: false,
        reason: `unmanaged path: ${record.path}`,
      };
    ...
    if (!branch.ok || branch.stdout.trim() !== record.taskBranch) {
      return {
        ok: false,
        clean: false,
        reason: `expected branch ${record.taskBranch}, found ${branch.stdout.trim() || "unknown"}`,
      };
    }
    const clean =
      git(["status", "--porcelain"], record.path).stdout.trim() === "";
  ...
  hasTaskDelta(record: TicketWorktreeRecord): boolean {
    if (!git(["fetch", "origin", record.planBranch], record.path).ok)
      return true;
    const result = git(
      [
        "rev-list",
        "--count",
        `origin/${record.planBranch}..${record.taskBranch}`,
      ],
      record.path,
    );
  ...
  /** Create or resume the persistent worktree for one board item. */
    ...
    if (!saved && this.has(task.itemId)) {
      throw new Error(
        `Ticket execution record is corrupt: ${this.recordPath(task.itemId)}`,
      );
    }
    if (saved) {
      if (!existsSync(saved.path))
        throw new Error(`Ticket worktree is missing: ${saved.path}`);
      if (!this.isManagedPath(saved.path))
        throw new Error(`Refusing unmanaged worktree path: ${saved.path}`);
      // Preserve the baseline recorded before direct-to-base merges were introduced.
      if (saved.taskBranch !== task.taskBranch || saved.plan !== plan) {
        throw new Error(
    ...
    if (registered) {
      throw new Error(
        `${task.taskBranch} is already checked out without a ticket record: ${registered}`,
      );
    }
    ...
    const path = join(
      this.worktreesDir,
      `ticket-${safe(task.taskKey)}-${suffix}`.slice(0, 64),
    );
    if (existsSync(path))
    ...
    if (
      !branchExists(task.taskBranch, this.repoRoot) &&
      remoteBranchExists(task.taskBranch, this.repoRoot)
    ) {
      mustGit(
        ["fetch", "origin", `${task.taskBranch}:refs/heads/${task.taskBranch}`],
        this.repoRoot,
      );
    }
  ...
  /** Merge a manually accepted ticket into the target branch, then remove its worktree. */
  mergeAndRemove(
    ...
    if (!this.isManagedPath(record.path))
      throw new Error(`Refusing unmanaged worktree path: ${record.path}`);
    if (!existsSync(record.path)) {
    ...
    if (git(["status", "--porcelain"], record.path).stdout.trim()) {
      throw new Error(
        `Ticket worktree has uncommitted changes: ${record.path}`,
      );
    }

    mustGit(["fetch", "origin", targetBranch, record.taskBranch], record.path);
    if (!remoteBranchExists(record.taskBranch, record.path)) {
      throw new Error(
        `Remote task branch does not exist: origin/${record.taskBranch}`,
      );
    }
    ...
    const remoteTask = mustGit(
      ["rev-parse", `origin/${record.taskBranch}`],
      record.path,
    );
    if (localTask !== remoteTask) {
      throw new Error(
        `${record.taskBranch} has local commits that were not reviewed and pushed`,
      );
    }

    const marker = `Board-Agent-Item: ${record.itemId}`;
    const marked = (branch: string) =>
      git(
        [
          "log",
          "-1",
    ...
    mustGit(["checkout", "--detach", `origin/${targetBranch}`], record.path);

    if (
      !marked("HEAD") &&
      !this.isIntegrated("HEAD", record.taskBranch, record.path)
    ) {
  ...
  private removePath(record: TicketWorktreeRecord): void {
    ...
    if (!result.ok && existsSync(record.path)) {
      if (this.isRegistered(record.path)) {
        throw new Error(
          `Failed to remove ticket worktree: ${result.stderr.trim()}`,
        );
      }
      rmSync(record.path, {
        recursive: true,
