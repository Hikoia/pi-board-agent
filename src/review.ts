...
const result = await agent(
  [
    ...
    '',
    'REVIEW PROCEDURE:',
    '1. Run \`git fetch origin --prune\`, then inspect origin/' + PAYLOAD.taskBranch + ' and origin/' + PAYLOAD.planBranch + '.',
    '2. Find the task commits from the task branch and issue reference. The task may already be squash- or merge-committed into the plan branch; an empty branch diff is not proof that nothing changed.',
    '3. Review the actual changed code against every acceptance criterion. Check correctness, regressions, security, error handling, and meaningful test coverage.',
    '4. Run the smallest relevant tests, typecheck, or lint commands supported by the repo.',
