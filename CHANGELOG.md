...
- Task-only execution: optional Plan, mandatory isolated exact-SHA AI review,
  merge-only native integration after human Issue closure. Done stays OPEN.
...
- Ordered cleanup verifies integration, expected remote deletion, normal worktree
  removal, expected local deletion, Project Done and record deletion last.
  Ignored task-worktree files are explicitly discardable by normal Git removal.
- Exclusive stopped/drained owner upgrade converts supported v3 Tasks without
  ...
  continue conservatively. Old ledgers/receipts remain read-only without GC.
