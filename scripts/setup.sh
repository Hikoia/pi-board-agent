...
PLAN_FIELD="$(ask 'Optional Plan field name (field/value need not exist)'  "${PLAN_FIELD:-Plan}")"
TYPE_FIELD="$(ask 'Type field name' "${TYPE_FIELD:-Type}")"
...
const statuses = [names[3], names[4], "In Progress", "Needs Human", "Review"].map(s => s.toLowerCase());
if (new Set(statuses).size !== statuses.length) process.exit(1);
' "$STATUS_FIELD" "$PLAN_FIELD" "$TYPE_FIELD" "$READY_COL" "$DONE_COL" || err 'Use non-empty single-line field names and five distinct status names.'
...
console.log(JSON.stringify({
  ...
  columns: { ready, building: "In Progress", needs_human: "Needs Human", review: "Review", done },
