...
| `test-cleanup-windows-lock.ts` | receipt creation before Windows cleanup | real PowerShell `FileShare.None` lock blocks Windows normal removal; original bytes/integration retained; safe retry after lock release; non-Windows skip is explicit |
...
| `test-builder-actual-start.ts`, `test-runtime-revision.ts` | filesystem/package polling on every admission/status | cached startup/lint revision admission/latch; actual-start stop and fresh board/capacity checks remain; explicit lint mismatch blocks admission |
