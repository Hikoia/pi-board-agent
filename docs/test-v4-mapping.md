...
| `test-cleanup-boundaries.ts` | receipts as new finalization transaction | atomic pre-push integration publication; lost/rejected push observation; fault cuts in remote/worktree/local/Backlog/record order; locks, dirty work, ref movement; no new snapshots/force/prune |
| `test-cleanup-link-races.ts`, `test-cleanup-preparation.ts` | integration-branch fixtures ported to the combined v4 executor | late ignored link/file/directory/parent/rule replacement; dirty/lock/local/base races after ignored clean and remote deletion; original refs/data/registration retained and cleanup-only retry |
...
| `test-cleanup-symlinks.ts` | unconditional successful native removal expectation | all legacy link/backup/corruption/immutable-evidence assertions retained; successful normal Git removal still required except the observed Windows junction residual: real normal removal loses registration while preserving the original link kind/target and external bytes; cleanup fails closed, and restart retains exact progress/refs/bytes without new receipts/snapshots/backups or speculative deletion. This is safe retention, not successful cleanup; other errors and non-Windows failures still fail. |
...
| `test-closed-done-refs.ts` | Task-only/local-only completion filter and mandatory review marker | all target Issue Types; custom Backlog; idle no-ref records/leftovers preserved; ref-query failures/races and pending/corrupt recovery fail closed; fresh authority and interrupted Backlog writes |
| `test-finalization-sources.ts`, `test-recordless-receipt.ts` | paired-ref prerequisite and receipt-only identity assumptions | local-only/remote-only/equal/ahead/divergent source ancestry; compare-and-create races; exact cleanup pins; immutable v1/v2 receipts including null-record migration and non-Task completion |
...
| `test-merge-conflict.ts` | conflict returned without ordinary writeback; legacy review SHA assumptions in fixture | Real stdout-only merge conflict classification, valid tree versus error/timeout/malformed output, exact task/base SHAs, preserved refs/work/record and no integration. Loop writeback and renewed approval moved to `test-conflict-handoff.ts`/`test-conflict-success.ts`, not removed. |
...
Main's completion contract takes precedence over the original T006 Task-only,
review-marker-required, stay-in-Done finalization assertions. Normal Task model
execution still runs isolated AI review; human completion is a separate authority.
...
## Retired repair-protocol runnable files
