...
try {
  for (const cut of ["prepare-save", "record-delete"] as const) {
    ...
    faults.beforeSyncFs = (op, path, destination) => {
      ...
      if (cut === "record-delete" && op === "unlinkSync" && path === f.recordFile) {
        assert.equal(f.card.status, f.cfg.columns.backlog); throw new Error("offline final record unlink failure");
      }
    };
    ...
    console.log(`PASS: ${cut} failure retains retry; push cannot precede atomic save and record deletion cannot precede fresh Project Backlog`);
  }
  {
  ...
  for (const prepared of [false, true]) {
    ...
    if (prepared) {
      ...
      const resultSha = git(f.repo, "commit-tree", tree, "-p", f.base, "-p", f.taskSha, "-m", "human-approved prepared result");
      f.store.update(f.task.itemId, (r) => ({ ...r, integration: { baseSha: f.base, taskSha: f.taskSha, resultSha } }));
    }
    calls.length = 0;
    assert.equal((await f.finish()).status, "finalized");
    assert.equal(f.store.has(f.task.itemId), false); assert.equal(f.card.status, f.cfg.columns.backlog);
    assert.equal(calls.filter((a) => a[0] === "commit-tree").length, prepared ? 0 : 1);
    console.log(`PASS: human-closed Done authorizes ${prepared ? "prepared" : "new"} integration without an AI-review marker`);
  }
  {
    const f = await fixture();
    ...
    console.log("PASS: manual closed-Done approval supersedes an idle AI-review retry without invoking a reviewer");
  }
  {
    const f = await fixture(); calls.length = 0;
  ...
  }
  {
    ...
    console.log("PASS: fresh withdrawal after Git deletion retains the ticket and never overwrites changed human Project status");
  }
} finally { dispose(); }
