...
try {
  for (const kind of [
    ...
    f.store.update(f.task.itemId, (r) => ({
      ...
      finalization: {
        targetBranch: "main",
        baseSha,
        taskSha: kind === "old-task-mismatch" ? f.base : f.taskSha,
        ...(kind === "unknown-result"
          ? { resultSha: "0".repeat(40) }
          : kind === "wrong-result"
            ? { resultSha: f.base }
            : {}),
      },
    ...
    try {
      ...
      assert.notEqual(
        (await f.executor.finalizeClosed(f.card)).status,
        "finalized",
      );
      ...
      console.log(
        `PASS: legacy ${kind} stays byte-identical and blocked without inventing integration or cleanup authority`,
      );
    } finally {
      owner.release();
    }
  }
  {
    ...
    try {
      ...
      assert.deepEqual(
        readFileSync(
          join(f.repo, ".pi/board-agent/legacy-v3", basename(f.recordFile)),
        ),
        bytes,
      );
