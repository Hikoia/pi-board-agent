...
try {
  ...
  if (process.platform !== "win32") {
    ...
    console.log("PASS: real registered worktree uses normal Git removal, no force/prune/unlock");
    console.log("SKIP: Windows FileShare.None exclusive lock requires Windows; deterministic partial-removal tests run on both platforms");
  } else {
    const path = join(f.record.path, "ignored", "cache.bin"), release = join(root, "release-exclusive-lock");
    ...
    faults.beforeGit = async (args) => {
      ...
      assert.ok(existsSync(f.receipt), "lock is acquired only after receipt publication and all pre-removal checks");
      ...
      child = spawn("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { stdio: ["ignore", "pipe", "pipe"] });
      let output = "", errors = "";
      closed = new Promise<void>((resolve, reject) => {
        child!.once("error", reject);
        child!.stderr!.on("data", (chunk) => { errors += chunk; });
        child!.once("close", (code) => code === 0 ? resolve() : reject(new Error(`lock helper exit ${code}: ${errors}`)));
      });
      void closed.catch(() => undefined);
      await Promise.race([
        new Promise<void>((resolve) => child!.stdout!.on("data", (chunk) => { output += chunk; if (output.includes("LOCKED")) resolve(); })),
        closed.then(() => { throw new Error("lock helper closed without acquiring lock"); }),
      ]);
      assert.throws(() => readFileSync(path), /EBUSY|EACCES|EPERM/, "real exclusive Windows lock denies a separate file read");
    };
    try {
      await assert.rejects(f.finish(), (error: unknown) => {
        assert.ok(error instanceof Error);
        assert.match(error.message, /git worktree remove .*failed/s, "failure must come from REAL Git, not an injected error");
        console.log(`Real locked Git removal diagnostic: ${error.message}`);
        return true;
      });
      assert.ok(child, "real exclusive lock was acquired");
      const integrated = f.tip(), receipt = readFileSync(f.receipt);
      ...
      writeFileSync(release, "release"); await closed;
      calls.length = 0;
      ...
      console.log("PASS: real Windows FileShare.None lock makes real Git removal fail; locked retry preserves evidence; release/restart completes exact cleanup without reintegration");
    } finally {
