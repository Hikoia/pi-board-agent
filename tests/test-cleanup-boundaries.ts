...
try {
  for (const kind of ["admin-index", "admin-locked", "common", "ref"] as const)
    for (const afterReceipt of [false, true]) {
      ...
      const lock =
        kind === "admin-index"
          ? join(f.admin, "index.lock")
          : kind === "admin-locked"
            ? join(f.admin, "locked")
            : kind === "common"
              ? join(f.repo, ".git", "index.lock")
              : join(
                  f.repo,
                  ".git",
                  "refs",
                  "heads",
                  `${f.task.taskBranch}.lock`,
                );
      if (afterReceipt) {
      ...
      assert.equal(
        git(f.origin, "rev-parse", `refs/heads/${f.task.taskBranch}`),
        f.taskSha,
      );
      assert.ok(
        !calls.some(
          (args) =>
            ...
            args[0] === "update-ref" ||
            (args[0] === "push" &&
              args.includes(`:refs/heads/${f.task.taskBranch}`)),
        ),
      );
      ...
      console.log(
        `PASS: ${kind} Git lock ${afterReceipt ? "after" : "before"} receipt blocks cleanup and retains evidence`,
      );
    }

  {
    ...
    assert.equal(f.store.localBranchSha(f.task.taskBranch), f.taskSha);
    console.log(
      "PASS: remote base mutation during cleanup cannot delete the local retry ref or announce success",
    );
  }

  {
    ...
    renameSync(join(f.repo, ".git"), saved);
    cpSync(saved, join(f.repo, ".git"), { recursive: true });
    calls.length = 0;
    await assert.rejects(f.finish(), /identity changed/);
    assert.ok(
      !calls.some((args) => ["fetch", "push", "update-ref"].includes(args[0])),
    );
    assert.ok(existsSync(f.receipt));
    assert.ok(existsSync(f.record.path));
    console.log(
      "PASS: replacing the Git common directory with an identical copy invalidates receipt ownership before any remote/local cleanup",
    );
  }

  {
    const f = await fixture(),
      integrated = legacy(f, "squash", "result");
    ...
    calls.length = 0;
    assert.equal(
      await f.finish("squash"),
      integrated,
      "known remote-confirmed old result means cleanup only, even if a fresh merge would conflict",
    );
    assert.equal(f.tip(), later);
    assert.equal(
      readFileSync(join(f.repo, "feature.txt"), "utf8"),
      "later legitimate base edit\n",
    );
    assert.ok(!calls.some((args) => args[0] === "commit-tree"));
    assert.equal(existsSync(f.record.path), false);
    console.log(
      "PASS: confirmed legacy squash result survives later conflicting main edits; prove the old result and clean only, without remerging or modifying main",
    );
  }

  {
    const f = await fixture();
    mkdirSync(join(f.record.path, "ignored", "bare", "objects"), {
      recursive: true,
    });
    mkdirSync(join(f.record.path, "ignored", "bare", "refs"));
    writeFileSync(
      join(f.record.path, "ignored", "bare", "HEAD"),
      "ref: refs/heads/main\n",
    );
    ...
    assert.equal(f.store.localBranchSha(f.task.taskBranch), f.taskSha);
    console.log(
      "PASS: nested bare Git identities are blocked even without a .git directory",
    );
  }

  {
    const f = await fixture();
    legacy(f, "merge", "result");
    const stale = join(f.repo, ".git", "worktrees", "ambiguous-admin");
    mkdirSync(stale, { recursive: true });
    writeFileSync(
      join(stale, "HEAD"),
      `ref: refs/heads/${f.task.taskBranch}\n`,
    );
    ...
    assert.equal(f.store.localBranchSha(f.task.taskBranch), f.taskSha);
    console.log(
      "PASS: unreceipted partial Git administration cannot be pruned, guessed or ignored before local-ref deletion",
    );
  }

  {
    ...
    assert.ok(
      !calls.some((args) =>
        ["fetch", "push", "merge-tree", "update-ref"].includes(args[0]),
      ),
    );
    ...
    assert.equal(
      ...
      false,
      "no early cleanup intent on a rejected integration",
    );
    assert.deepEqual(readFileSync(f.recordFile), original);
    unlinkSync(remoteHook);
    writeFileSync(
      join(f.origin, "hooks", "post-receive"),
    ...
    assert.equal(
      existsSync(f.receipt),
      false,
      "even a successful push is not remote integration confirmation",
    );
    assert.deepEqual(readFileSync(f.recordFile), original);
    assert.ok(existsSync(f.record.path));
    console.log(
      "PASS: mixed v3 corruption, rejected push and unconfirmed successful push publish no receipt and preserve all source evidence",
    );
    ...
    assert.equal(existsSync(f.receipt), false);
    console.log(
      "PASS: a local branch does not authorize deletion of an unrecorded managed directory",
    );
  }

  {
    ...
    faults.beforeFs = (operation, path) => {
      if (operation === "link" && path.startsWith(f.receipt))
        throw new Error("atomic publication refused");
    };
    ...
    assert.equal(
      await f.finish(),
      integrated,
      "publication failure must recover with a legacy backup, never duplicate integration",
    );
    assert.equal(existsSync(f.record.path), false);
    assert.ok(
      existsSync(join(f.repo, ".pi", "board-agent", "cleanup-backups")),
    );
    console.log(
      "PASS: failed atomic publication leaves no partial receipt or cleanup; restart backs up the integrated legacy residual before retry",
    );
  }

  {
    ...
    assert.ok(
      !calls.some((args) =>
        ["push", "commit-tree", "update-ref"].includes(args[0]),
      ),
    );
    assert.equal(existsSync(f.receipt), false);
    assert.ok(existsSync(f.record.path));
    console.log(
      "PASS: an ancestor OID alone cannot turn a corrupt legacy result into integration/cleanup authority",
    );
  }

  {
    ...
    let injected = false,
      ...
      scanYielded = false;
    faults.beforeFs = (operation, path) => {
      if (operation === "readdir" && path === join(f.record.path, "ignored"))
        setImmediate(() => {
          timerRan = true;
        });
    };
    ...
    faults.beforeGit = (args) => {
      if (
        ...
        injected = true;
        writeFileSync(
          join(f.record.path, "ignored", "new-after-await"),
          "new user data",
        );
      }
    };
    ...
    assert.equal(
      scanYielded,
      true,
      "full ignored snapshot yields before publication, not a synchronous traversal",
    );
    assert.ok(
    ...
    assert.equal(f.store.localBranchSha(f.task.taskBranch), f.taskSha);
    assert.equal(
      readFileSync(join(f.record.path, "ignored", "new-after-await"), "utf8"),
      "new user data",
    );
    assert.ok(existsSync(f.receipt));
    console.log(
      "PASS: async ignored-file snapshot yields, and mutations during remote cleanup await block normal Git removal and retain receipt/ref",
    );
  }
} finally {
  dispose();
