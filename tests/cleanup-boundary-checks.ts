...
import { calls, faults, git } from "./cleanup-fixture.js";

// Keep native Git fixture batches bounded; test files still run serially.
export async function cleanupBoundaryChecks(boundary: "ignored clean" | "remote deletion") {
  for (const race of ["dirty", "lock", "local", "base"] as const) {
    ...
    console.log(`PASS: ${race} race during ${boundary} await blocks native removal; cleanup-only retry preserves code/locks/refs/approval and never remerges`);
  }
}
