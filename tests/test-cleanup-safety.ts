...
try {
  {
    ...
    assert.ok(
      calls.some(
        (a) => a[0] === "worktree" && a[1] === "remove" && a.length === 3,
      ),
    );
    ...
    console.log(
      "PASS: Cargo.lock, locked and ignored node_modules lockfiles permit normal removal; ignored files are explicitly discardable without new snapshots",
    );
  }
  ...
  for (const change of [
    ...
    assert.equal(
      calls.some((a) => ["push", "commit-tree", "update-ref"].includes(a[0])),
      false,
    );
  ...
  console.log(
    "PASS: migration rejects strict receipt version/identity/snapshot corruption, including missing local ref, without guessing cleanup authority",
  );
  ...
  const copy = join(backup, "0", "ignored", "cache.bin"),
    copyBytes = readFileSync(copy);
  ...
  await assert.rejects(finish(), /unmanaged|symlink/i);
  unlinkSync(f.record.path);
  renameSync(saved, f.record.path);
  const nested = join(f.record.path, "ignored", "nested");
  symlinkSync(root, nested, "junction");
  await assert.rejects(finish(), /symlink/i);
  unlinkSync(nested);
  mkdirSync(join(f.record.path, "ignored", ".git"));
  await assert.rejects(finish(), /Nested Git/);
  rmSync(join(f.record.path, "ignored", ".git"), { recursive: true });
  writeFileSync(join(f.repo, ".git", "index.lock"), "lock");
  await assert.rejects(finish(), /Locked/);
  unlinkSync(join(f.repo, ".git", "index.lock"));
  assert.deepEqual(readFileSync(f.receipt), receipt);
  assert.equal(f.tip(), tip);
  console.log(
    "PASS: migrated residual cleanup preserves changed backup/source, ignored additions, file/directory replacements, junctions, nested Git and Git locks",
  );
  let mutated = false;
  faults.afterFs = (op, path) => {
    if (!mutated && op === "unlink" && path.startsWith(f.record.path)) {
      mutated = true;
  ...
  unlinkSync(added);
  assert.equal(await finish(), tip);
  assert.equal(existsSync(f.record.path), false);
  assert.deepEqual(readFileSync(f.receipt), receipt);
  assert.ok(existsSync(backup));
  assert.equal(existsSync(f.recordFile), false);
  console.log(
    "PASS: existing legacy evidence rechecks each residual deletion, resumes partial removal without reintegration, and remains archived/read-only with no GC",
  );
} finally {
  dispose();
}
