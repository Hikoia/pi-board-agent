...

{
  ...
  const tick = async (warnings: number) => {
    ...
    assert.ok(
      commentReads > before.commentReads,
      "every tick retries handoff confirmation",
    );
    assert.equal(
      f.warnings.length - before.warnings,
      warnings,
      f.notices.join("\n"),
    );
    assert.ok(
      f.warnings.every((w) => w.source === "loop"),
      "executor returns blocking outcomes; only loop warns",
    );
  ...
  try {
    ...
    console.log(
      "PASS: three real ticks retry an unchanged handoff blocker but warn once in the loop, not the executor or alternating wrappers",
    );
    ...
    await tick(0);
    console.log(
      "PASS: changed/returning handoff reasons and a new loop lifetime warn again without suppressing fresh retries",
    );

    const eligible = structuredClone(f.card);
    for (const patch of [
      { status: f.cfg.columns.needs_human },
      { closed: false },
      { repoOwner: "other" },
      { number: undefined },
    ...
    await f.loop.tickNow();
    assert.equal(f.warnings.length, before);
    f.cards.push(f.card);
    await tick(1);
    await tick(0);
    console.log(
      "PASS: irrelevant handoff entries are pruned for changed lane/open/type/repository/identity and absent tickets",
    );

    const other = {
      ...f.card,
      itemId: "HANDOFF_OTHER",
      number: f.card.number! + 100,
    };
    ...
    console.log(
      "PASS: two tickets with identical handoff SHAs/reasons have independent loop blocker fingerprints",
    );
  } finally {
    await f.loop.stop();
  }
...

{
  ...
  try {
    ...
    assert.equal(
      f.warnings.length,
      2,
      "successful same-tick handoff clears its earlier blocker before an identical new failure",
    );
    ...
    assert.equal(f.runs().length, 0);
    console.log(
      "PASS: a successful fresh handoff retry in the same tick clears the reconcile blocker before the identical later failure",
    );
  } finally {
    await f.loop.stop();
  }
  ...
  for (const boundary of ["comments", "card"] as const) {
    ...
    try {
      for (let tick = 0; tick < 3; tick++) {
        ...
        await f.loop.tickNow();
        assert.ok(reads > before.reads);
        assert.ok(commentReads > before.commentReads);
      ...
      assert.deepEqual(f.events, [
        ...
        ...(state === "open Ready" ? ["reopen"] : []),
      ]);
      console.log(
        `PASS: interrupted ${state} ${boundary} reads retry on three ticks with one warning across finalizer/reconcile/Ready wrappers and no repeated write/builder`,
      );
    } finally {
      await f.loop.stop();
    }
...

{
  ...
  try {
    ...
    let commentsRead = 0;
    f.setHook((event) => {
      if (event === "read:comments") commentsRead++;
    });
    ...
    for (let tick = 1; tick <= 3; tick++) {
      ...
      assert.equal(
        commentsRead,
        tick,
        "Ready-only marker authorization is refreshed every tick",
      );
      assert.equal(f.warnings.length, 1, f.notices.join("\n"));
    }
    assert.equal(f.warnings[0].source, "loop");
    ...
    console.log(
      "PASS: Ready-only unrecognized authentic marker warns once over three real retries with no ordinary builder fallback",
    );
    ...
    const marker = f.comments.pop()!;
    let authorized = false;
    f.setHook((event) => {
      if (event === "read:comments") authorized = true;
      if (event === "read:card" && authorized)
    ...
    f.setHook(() => {});
    f.card.status = f.cfg.columns.ready;
    f.comments.push(marker);
    await f.loop.tickNow();
    await f.loop.tickNow();
    assert.equal(f.warnings.length, 2);
    assert.equal(f.warnings[0].message, f.warnings[1].message);
    console.log(
      "PASS: recovered Ready authorization clears the blocker so the identical later failure warns anew",
    );
  } finally {
    await f.loop.stop();
  }
...

{
  ...
  f.setHook((event) => {
    ...
    if (event === "read:card" && ++reads === 3)
      throw new Error("handoff fresh card unavailable");
  });
  const tick = async (warnings: number) => {
    reads = 0;
    await f.loop.tickNow();
    assert.equal(reads, 3);
    assert.equal(f.warnings.length, warnings, f.notices.join("\n"));
    assert.ok(f.warnings.every((w) => w.source === "loop"));
  };
  try {
    const file = join(
    ...
    await tick(1);
    await tick(1);
    git(f.repo, "commit", "--allow-empty", "-m", "new base, same conflict");
    ...
    git(
      ...
      "--allow-empty",
      "-m",
      "new task, same conflict",
    );
    git(f.record.path, "push", "origin", f.task.taskBranch);
    await tick(3);
    await tick(3);
    assert.equal(
      new Set(f.warnings.map((w) => w.message)).size,
      1,
      "reason/wording unchanged; only actual SHAs changed",
    );
    assert.equal(f.calls(), 0);
    assert.equal(f.runs().length, 0);
    assert.deepEqual(f.events, []);
    ...
    console.log(
      "PASS: advancing either real conflict SHA warns again even when the handoff blocker reason is identical",
    );
  } finally {
    await f.loop.stop();
  }
