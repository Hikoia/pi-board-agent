...
import { fixture, legacy, calls, faults, TicketWorktrees, dispose } from "./cleanup-fixture.js";
try {
  {
    ...
    console.log("PASS: unrecorded empty directories in legacy residue block backup authorization and cleanup");
  }

  {
    ...
    assert.notEqual(f.tip(), f.base, "integration was confirmed before cleanup");
    assert.equal(f.store.localBranchSha(f.task.taskBranch), f.taskSha);
    assert.deepEqual(readFileSync(f.recordFile), recordBytes, "no early intent or v3 change");
    assert.ok(existsSync(f.receipt), "failed removal must retain a dedicated cleanup receipt");
    ...
    assert.equal(existsSync(f.record.path), false, "lost registration and .git cannot hide residual files");
    ...
    console.log("PASS: confirmed integration writes a full dedicated receipt; restart cleans verified registration-less leftovers before ref/record/receipt without reintegration");
  }

  for (const last of ["record", "receipt"] as const) {
    const f = await fixture();
    faults.beforeFs = (operation, path) => {
      if (operation === "unlink" && path === (last === "record" ? f.recordFile : f.receipt)) throw new Error(`controlled ${last} unlink failure`);
    };
    await assert.rejects(f.finish(), new RegExp(`controlled ${last} unlink failure`));
    ...
    const card: Card = { itemId: f.task.itemId, number: f.task.issueNumber, type: "Task", contentType: "Issue", title: "Accepted", body: "acceptance", repoOwner: "owner", repoName: "repo", assignees: [], closed: true, status: cfg.columns.done };
    const notices: string[] = [];
    const callback = (message: string) => { notices.push(message); };
    const noWrite = async (): Promise<never> => assert.fail("cleanup does not mutate the card or launch repair");
    const board: TicketBoardAdapter = { getCard: async () => structuredClone(card), claim: noWrite, release: noWrite, comment: noWrite, listComments: noWrite, setStatus: noWrite };
    const store = new TicketWorktrees(f.repo);
    const executor = new ManagedTicketExecutor({ cwd: f.repo, cfg, worktrees: store, board, botLogin: "bot", repoOwner: "owner", repoName: "repo", callback, createManager: () => assert.fail("no builder") });
    const loop = new BoardLoop({ cwd: f.repo, cfg, botLogin: "bot", repoOwner: "owner", repoName: "repo", callback, meta: { projectId: "P", statusFieldId: "S", statusOptions: {} }, listCards: async () => [structuredClone(card)] }, createLoopState(), executor, store);
    try { await loop.tickNow(); } finally { await loop.stop(); }
    assert.equal(existsSync(f.receipt), false, `no-ref negative filter must retain pending cleanup: ${JSON.stringify(notices)}`);
    assert.equal(existsSync(f.recordFile), false);
    assert.equal(f.tip(), integrated);
    assert.equal(notices.filter((message) => message.startsWith("Finalized")).length, 1);
    assert.ok(!calls.some((args) => ["merge-tree", "commit-tree"].includes(args[0])));
    console.log(`PASS: ${last} unlink failure retains a no-ref receipt; restarted real loop/executor completes it without reintegration`);
  }
} finally { dispose(); }
