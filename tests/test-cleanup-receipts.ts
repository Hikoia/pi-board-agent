...
try {
  {
    ...
    faults.beforeGit = (a) => {
      if (a[0] === "worktree" && a[1] === "remove") {
        ...
        f.vanish();
        throw new Error("partial worktree removal");
      }
    };
    ...
    assert.equal(
      calls.some((a) =>
        ["merge-tree", "commit-tree", "update-ref"].includes(a[0]),
      ),
      false,
    );
    console.log(
      "PASS: partial normal removal retains v4 cleanup progress, unknown files/directories and refs; no new snapshot, backup or recursive fallback can invent ownership",
    );
  }
  {
    const f = await fixture(false, true);
    faults.beforeSyncFs = (op, path) => {
      if (op === "unlinkSync" && path === f.recordFile)
        throw new Error("record unlink failure");
    };
    await assert.rejects(f.finish(), /record unlink failure/);
    ...
    console.log(
      "PASS: final record unlink failure resumes after missing refs/path without a duplicate merge; the single v4 record is deleted last",
    );
  }
  {
    ...
    faults.beforeFs = (op, path) => {
      if (!cut && op === "unlink" && path.startsWith(f.record.path)) {
        cut = true;
        throw new Error("legacy partial unlink");
      }
    };
    await assert.rejects(finish(), /legacy partial unlink/);
    assert.ok(cut);
    ...
    assert.equal(existsSync(f.recordFile), false);
    console.log(
      "PASS: pre-existing receipted legacy residual resumes partial cleanup without reintegration or deleting/rewriting its archived source",
    );
  }
} finally {
  dispose();
