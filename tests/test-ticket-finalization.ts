...
try {
  ...
  }
  {
    ...
    faults.beforeGit = (args) => {
      ...
      assert.deepEqual(f.recordNow().integration, { baseSha: advanced, taskSha: f.taskSha, remoteTaskSha: f.taskSha, resultSha: args.find((s) => s.endsWith(":refs/heads/main"))!.split(":")[0] });
    };
  ...
  for (const step of ["remote", "local", "worktree", "done", "lost-done"] as const) {
    ...
    assert.equal(f.starts(), 0); assert.equal(f.reviews(), 0); f.noNewEvidence();
    console.log(`PASS: ${step} I/O cut retains record/closed approval and resumes ordered cleanup/Backlog only, with missing refs/path handled safely`);
  }
