...
try {
  ...
  assert.deepEqual(f.events, ["claim", "ordinary-comment", "reopen", "status:Ready", "release"]);
  assert.equal(f.calls(), 0, "no immediate same-ticket retry in the conflict tick");
  await f.loop.tickNow(); await settle(f);
  const run = f.runs()[0];
  assert.equal(f.runs().length, 1);
  assert.equal((run.args as any).repair, undefined);
  await f.loop.tickNow();
  assert.equal(f.card.status, f.cfg.columns.ready, "technical failure retries build, never Needs Human");
  ...
  console.log("PASS: real conflict reopens Ready on original worktree; staged technical retry uses one normal builder without repair markers/ledgers or same-tick replay");
