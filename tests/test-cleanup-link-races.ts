...
try {
  for (const race of ["file", "directory", "parent", "ignore rule"] as const) {
    const f = await fixture(), ignored = join(f.record.path, "ignored"), link = join(ignored, "link");
    ...
    // Real late-created link survives the native clean on either OS. The same
    // fallback handles repository junctions skipped by Git for Windows.
    faults.afterGit = (a, r) => {
      if (a[0] !== "clean") return;
    ...
    await assert.rejects(f.store.finalizeAccepted(f.task, "merge", async () => {
      // The existing approval guard runs after the metadata walk, immediately
      // before link handling. Mutate only then, not before validation sees it.
    ...
    if (race === "ignore rule") assert.ok(lstatSync(link).isSymbolicLink(), "no longer ignored; do not unlink");
