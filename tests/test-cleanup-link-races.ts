...
try {
  for (const race of ["file", "directory", "parent", "ignore rule"] as const) {
    const f = await fixture(false, true),
      ignored = join(f.record.path, "ignored"),
      link = join(ignored, "link");
    ...
    faults.afterGit = (a) => {
      ...
      faults.afterFs = (op, path) => {
        if (op === "readdir" && path === ignored) scanned = true;
      };
    };
    calls.length = 0;
      ...
      simulateHumanFinalization(f.store, f.task, async () => {
        ...
        if (race === "file" || race === "directory") {
          ...
          if (race === "file") writeFileSync(link, "replacement data\n");
            ...
            mkdirSync(link);
            writeFileSync(join(link, "new.txt"), "replacement data\n");
          }
        } else if (race === "parent") {
    ...
    if (race === "file")
      assert.equal(readFileSync(link, "utf8"), "replacement data\n");
    if (race === "directory")
      assert.equal(
        readFileSync(join(link, "new.txt"), "utf8"),
        "replacement data\n",
      );
    ...
    console.log(
      `PASS: ignored link ${race} race preserves replacement/external data and registered ownership/record/refs before native removal`,
    );
  }
} finally {
  dispose();
