...
import { summarizePlans } from "../src/plan.js";
...
check(_DEFAULTS.models.review === "deepseek-v4-flash-0731" && !("enabled" in _DEFAULTS.review), "AI review is unconditional with its configured model");
...
check(auth.doneCards === 2, "auth summary counts case-insensitive Done but is incomplete at 2/3");
...
check(_DEFAULTS.telegram.on.includes("needs_human") && !_DEFAULTS.telegram.on.includes("ci_fixed") && !_DEFAULTS.telegram.on.includes("pr_opened"), "Telegram retains live events and omits obsolete lane events");
