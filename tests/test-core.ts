...
check(
  statuses.length === 7 && statuses.includes("Needs Human"),
  "init-project creates seven statuses including Needs Human",
);
check(
  !_DEFAULTS.review.enabled &&
    _DEFAULTS.models.review === "deepseek-v4-flash-0731",
  "AI review defaults disabled with its configured model",
);
...
const backlogged = summarizePlans(cfg, [
  card(1, { status: cfg.columns.backlog, closed: true }),
  card(2, { status: cfg.columns.done }),
]).get("001-auth")!;
check(
  isPlanComplete(backlogged),
  "closed Backlog retains Plan completion counts",
);
check(
  !isPlanComplete(
    summarizePlans(cfg, [card(1, { status: cfg.columns.backlog })]).get(
      "001-auth",
    )!,
  ),
  "open Backlog is not Plan completion",
);
...
const good: RefineOutput = {
  goal: "Add password reset",
  impactedAreas: ["src/auth"],
  decisions: ["use existing auth API"],
  risks: ["rate limit"],
  openQuestions: [],
  tasks: [
    {
      title: "Add form",
      acceptanceCriteria: ["validates email", "sends link"],
    },
  ],
};
...
check(
  parseRefineOutput(null) === null &&
    parseRefineOutput({ ...good, goal: 42 }) === null,
  "refine parser rejects null and non-string goals",
);
check(
  parseRefineOutput({ ...good, tasks: "nope" }) === null,
  "refine parser rejects malformed tasks instead of creating a partial plan",
);
...
check(
  parseRefineOutput({ ...good, tasks: many })?.tasks.length === 12 &&
    parseRefineOutput({ ...good, tasks: [...many, many[0]] }) === null,
  "refine accepts 12 tasks but fails closed above the cap (no silent truncation)",
);
const refine = await execute(
  renderRefineWorkflowSource({
    cwd: repo,
    storyTitle: "Add password reset",
    storyBody: "Users need password reset",
    maxTasks: cfg.refine.max_tasks,
    extraContext: "Use email links",
    contextDigest: tricky,
    model: "refine-model",
    timeoutMs: 240000,
  }),
  good,
...
check(
  refine.prompt.includes("Add password reset") &&
    refine.prompt.includes(tricky) &&
    refine.prompt.includes("Use email links") &&
    refine.options?.model === "refine-model" &&
    schemaHas(refine, "openQuestions"),
  "executed refinement receives story, context, human answers, model, and schema",
);
check(
  refine.prompt.includes("MINIMALISM:") &&
    refine.prompt.includes("fewest complete tasks"),
  "refinement retains minimal design policy",
);
const questions = renderQuestionsComment("001-auth", {
  ...good,
  openQuestions: ["q1?", "q2?"],
});
check(
  questions.includes("Needs Design") &&
    questions.includes("1. q1?") &&
    questions.includes("2. q2?"),
  "Needs Design comment numbers every open question",
);
const refined = renderRefineComment("001-auth", good, [
  { number: 12, url: "https://example.invalid/12", taskKey: "T001" },
]);
check(
  refined.includes("T001") &&
    refined.includes("[#12](https://example.invalid/12)") &&
    refined.includes("use existing auth API"),
  "refinement comment links created tasks and records decisions",
);
...
check(
  watchdog.fixAttempts === 2 &&
    watchdog.lastFixAtMs === 1234 &&
    !watchdog.needsHuman,
  "watchdog state survives a new store instance per PR",
);
check(
  store.get(999).fixAttempts === 0 && !store.get(999).needsHuman,
  "watchdog defaults for an unknown PR",
);
const fix = await execute(
  renderFixWorkflowSource({
    prNumber: 42,
    repoOwner: "test",
    repoName: "repo",
    headBranch: "task/issue-42",
    headSha: taskSha,
...
check(
  fix.prompt.includes("PR #42") &&
    fix.prompt.includes(taskSha) &&
    fix.prompt.includes("pr-ci, branch-ci") &&
    fix.prompt.includes("test/repo") &&
    fix.options?.model === "fix-model",
  "CI fix prompt executes with PR, pinned SHA, checks, repo, and model",
);
check(
  fix.prompt.includes("MINIMALISM:") &&
    fix.prompt.includes("narrowest shared seam"),
  "CI fix retains minimal root-cause policy",
);
...
check(
  reply.includes("PR #7") &&
    reply.includes("@board-bot what is the plan?") &&
    reply.includes("required: ['reply']"),
  "reply workflow embeds the mention and reply schema",
);
check(
  reply.includes("MINIMALISM:") && reply.includes("smallest action"),
  "reply prompt avoids speculative redesign",
);
...
check(
  ...
  "Telegram retains live events and removes the obsolete plan-PR event",
);
