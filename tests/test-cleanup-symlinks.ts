...
try {
  ...
  for (const link of [
    ...
    try {
      symlinkSync(link.target, join(f.record.path, link.path), link.kind);
      links.push(link);
    } catch (error) {
      if (
  ...
  if (process.platform === "win32") {
    const junction = {
      path: "ignored/junction",
      target: outside,
      kind: "junction",
    } as const;
    symlinkSync(
      junction.target,
      join(f.record.path, junction.path),
      junction.kind,
    );
    links.push(junction);
  }
  if (!nativeSymlinks)
    console.log(
      "NOTE: Windows lacks native symlink privilege; real junction coverage runs here, file/directory/dangling symlinks require the Linux run.",
    );
  if (nativeSymlinks) {
    const tracked = {
      path: "tracked-link",
    ...
    symlinkSync(
      tracked.target,
      join(f.record.path, tracked.path),
      tracked.kind,
    );
    links.unshift(tracked);
    git(f.record.path, "config", "core.symlinks", "true");
    git(f.record.path, "add", tracked.path);
    git(f.record.path, "commit", "-m", "tracked symlink");
    git(f.record.path, "push", "origin", f.task.taskBranch);
    f.taskSha = git(f.record.path, "rev-parse", "HEAD");
    assert.match(
      git(f.repo, "ls-tree", f.taskSha, "--", tracked.path),
      /^120000 blob /,
    );
  }
  ...
  if (nativeSymlinks) {
    const tracked = join(f.record.path, "tracked-link"),
      original = join(outside, "original-tracked-link");
    ...
    unlinkSync(tracked);
    symlinkSync("wrong-target", tracked, "file");
    await assert.rejects(finish(), /changed/);
    unlinkSync(tracked);
    renameSync(original, tracked);
    const regular = join(f.record.path, "base.txt"),
      originalRegular = join(outside, "original-base.txt");
    renameSync(regular, originalRegular);
    symlinkSync(originalRegular, regular, "file");
    await assert.rejects(finish(), /changed/);
    unlinkSync(regular);
    renameSync(originalRegular, regular);
    ...
    assert.ok(
      !calls.some(
        (args) =>
          args[0] === "hash-object" &&
          args.some((arg) => arg.startsWith(f.record.path)),
      ),
      "never ask Git to hash a residual path that could be a link",
    );
    console.log(
      "PASS: legacy tracked links require Git mode 120000 and the exact target blob; file/link mode ambiguity and changed targets block before receipt or cleanup",
    );
  }
  ...
  for (const invalid of [
    { target: null },
    { linkType: "unsupported-reparse" },
    { sha256: "a".repeat(64) },
  ]) {
    const corrupt = structuredClone(receipt);
    Object.assign(
      corrupt.snapshots[0].entries.find(
        (entry: any) => entry.path === links[0].path,
      ),
      invalid,
    );
  ...
  const source = join(f.record.path, links[0].path),
    saved = join(outside, "original-link");
  ...
  faults.beforeFs = (operation, path) => {
    if (operation === "readlink" && path === source)
      throw Object.assign(new Error("unsupported reparse readlink"), {
        code: "EINVAL",
      });
  };
  ...
  faults.afterFs = (operation, path) => {
    if (!changedDuringRead && operation === "readlink" && path === source) {
      ...
      renameSync(source, saved);
      symlinkSync(join(outside, ".git"), source, links[0].kind);
    }
  };
  await assert.rejects(finish(), /changed|replaced/i);
  ...
  symlinkSync(join(outside, ".git"), backupLink, links[0].kind);
  await assert.rejects(finish(), /backup/i);
  assert.ok(existsSync(f.record.path));
  unlinkSync(backupLink);
  symlinkSync(links[0].target, backupLink, links[0].kind);
  ...
  faults.beforeFs = (operation, path) => {
    if (links.some((link) => path === join(f.record.path, link.path))) {
      assert.ok(
        !["open", "readdir"].includes(operation),
        "never read/traverse the target",
      );
  ...
  assert.deepEqual(
    unlinked.sort(),
    links.map((link) => join(f.record.path, link.path)).sort(),
    "each source link is unlinked only",
  );
  assert.equal(
    readFileSync(targetFile, "utf8"),
    "outside target edited after receipt\n",
  );
  assert.ok(
    existsSync(join(outside, ".git")),
    "a target's Git identity is not traversed",
  );
  for (const link of links)
    assert.equal(
      readlinkSync(join(receipt.backup, "0", link.path)),
      link.target,
    );
  assert.ok(
  ...
  assert.deepEqual(readFileSync(f.receipt), receiptBytes);
  assert.deepEqual(
    readFileSync(join(receipt.backup, "verified.json")),
    verifiedBytes,
  );
  assert.equal(f.store.localBranchSha(f.task.taskBranch), undefined);
  console.log(
    `PASS: original child ${nativeSymlinks ? "file/directory/dangling symlinks" : "Windows junction"} survive receipt and verified backup; changed/replaced links block; cleanup unlinks links without touching targets`,
  );

  faults.beforeFs = undefined;
  const registered = await fixture(false, true);
  const registeredTarget = join(
    registered.repo,
    ".pi",
    "registered-link-target",
  );
  mkdirSync(registeredTarget);
  const targetBytes = Buffer.from(
    "normal Git removal must not follow this link\n",
  );
  ...
  const originalLink = await readSymlink(child),
    originalIdentity = lstatSync(child, { bigint: true });
  assert.deepEqual(originalLink, {
    target: registeredTarget,
    linkType: process.platform === "win32" ? childKind : "file",
  });
  ...
  faults.beforeGit = (args) => {
    if (args[0] === "worktree" && args[1] === "remove") {
      ...
      assert.equal(
        existsSync(registered.receipt),
        false,
        "no new snapshot/receipt",
      );
      assert.ok(
        registered.store
          .worktreeEntries()
          .some(
            (entry) => resolve(entry.path) === resolve(registered.record.path),
          ),
      );
  ...
  try {
    ...
    if (
      process.platform !== "win32" ||
      originalLink.linkType !== "junction" ||
      !(error instanceof Error) ||
      error.name !== "Error" ||
      error.message !== "Unknown unregistered worktree residual retained."
    )
      throw error;
    retained = true;
  }
  ...
  assert.deepEqual(
    calls.filter((args) => args[0] === "worktree" && args[1] === "remove"),
    [["worktree", "remove", registered.record.path]],
  );
  const assertTargetAndNoEvidence = () => {
    assert.deepEqual(
      readFileSync(join(registeredTarget, "keep.txt")),
      targetBytes,
    );
    assert.equal(existsSync(registered.receipt), false);
    assert.deepEqual(
      readdirSync(join(registered.repo, ".pi", "board-agent", "cleanup")),
      [],
    );
    assert.equal(
      existsSync(
        join(registered.repo, ".pi", "board-agent", "cleanup-backups"),
      ),
      false,
    );
  };
  if (retained) {
    ...
    assert.deepEqual(
      registered.store.read(registered.task.itemId)?.integration,
      {
        baseSha: registered.base,
        taskSha: registered.taskSha,
        remoteTaskSha: registered.taskSha,
        resultSha: integrated,
      },
    );
    assert.equal(
      registered.store.read(registered.task.itemId)?.retry?.stage,
      "cleanup",
    );
    const assertRetained = async () => {
      ...
      assert.equal(
        registered.store.localBranchSha(registered.task.taskBranch),
        registered.taskSha,
      );
      assert.equal(
        git(
          ...
          "for-each-ref",
          "--format=%(objectname)",
          `refs/heads/${registered.task.taskBranch}`,
        ),
        "",
      );
      assert.equal(
    ...
    assert.ok(
      !calls.some(
        (args) =>
          ["merge-tree", "commit-tree", "push", "update-ref"].includes(
            args[0],
          ) ||
          (args[0] === "worktree" && args[1] === "remove"),
      ),
      "restart must not reintegrate or mutate refs/worktree",
    );
    console.log(
      "PASS: safe retention, NOT successful cleanup: real Windows normal Git removal lost registration but retained the original junction; restart preserves exact progress/refs/target bytes without new evidence or speculative deletion",
    );
  } else {
    assert.equal(existsSync(registered.record.path), false);
    assertTargetAndNoEvidence();
    console.log(
      "PASS: fresh integration with no receipts retains external child-link targets and normal registered Git removal never deletes their targets",
    );
  }
} finally {
  dispose();
