...
try {
  ...
  function fixture(numbers: number[], prefix = "task/", createManager: () => TicketWorkflowManager = () => { throw new Error("unexpected model manager"); }) {
    ...
    const addBranch = async (number: number) => {
      const task = buildTasksForWave(cfg, "", [cards.find((c) => c.number === number)!])[0];
      const record = await store.ensure(task);
      git(record.path, "commit", "--allow-empty", "-m", `accepted task ${number}`);
      const sha = git(record.path, "rev-parse", "HEAD");
      git(record.path, "push", "origin", record.taskBranch);
      store.setReviewedTaskSha(task.itemId, sha);
      return sha;
    };
  ...
  for (const prefix of ["task/", "ops/task-"]) {
    ...
    try {
      ...
      assert.ok(f.reads.length > 1 && f.reads.every((id) => id === "ITEM_7"), "only the complete local branch name qualifies; no suffix, prefix, tag or remote matches");
      ...
      assert.deepEqual(f.store.list(), [], "reviewed optional-Plan execution record is removed only after integration");
  ...
  for (const change of ["reopened", "status", "identity", "removed", "unreadable", "vanished"] as const) {
    ...
    afterRefs = () => { afterSnapshot = true; };
    f.board.getCard = async (id) => {
      const fresh = await read(id);
      if (!afterSnapshot) return fresh;
...

  {
    ...
    try {
      await f.loop.tickNow();
      assert.equal(refQueries(), 1);
      assert.deepEqual(f.reads, ["ITEM_1"], "reconcile still reads the retained record");
      ...
      assert.ok(f.reads.length >= 2 && f.reads.every((id) => id === "ITEM_1"));
...

  {
    ...
    try {
      ...
      assert.deepEqual(f.reads, ["ITEM_1"], "only reconcile read; no finalizer after stop during refs query");
      assert.equal(f.tip(), f.base);
      assert.ok(f.store.localBranchSha(f.branch(1)));
...

  {
    ...
    try {
      for (let tick = 1; tick <= 2; tick++) {
        await f.loop.tickNow();
        assert.equal(refQueries(), 0, "unsettled execution is excluded from finalization on this tick");
        assert.equal(f.reads.length, tick, "no-branch filter must NOT skip reconcile's fresh read of unsettled records");
        assert.equal(releases, 0, "missing original run cannot prove drain or authorize release");
