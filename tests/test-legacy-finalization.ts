...
try {
  ...
  }
  {
    ...
    try {
      ...
      f.store.update(f.task.itemId, (r) => ({
        ...r,
        lastRunId: "new-v4-build-without-review",
      }));
      ...
      console.log(
        "PASS: fresh human closure approves an idle converted v4 execution without inheriting or fabricating AI-review evidence",
      );
    } finally {
      owner.release();
    }
  }
  {
    ...
    try {
      assert.deepEqual((await adapter(f).migrate(owner)).failures, []);
      assert.equal(f.recordNow().integration?.resultSha, resultSha);
      ...
      assert.equal(f.reviews(), 0);
      console.log(
        "PASS: unconfirmed legacy squash on an advanced base retries as one ordinary merge of the original approved task, without a forged review marker or renewed model/close cycle",
      );
    } finally {
      owner.release();
    }
  ...
  for (const fullyRemoved of [false, true]) {
    ...
    try {
      ...
      f.store.update(f.task.itemId, (r) => ({
        ...
        retry: {
          stage: "cleanup",
          reason: "new v4 retry may update beside immutable old receipt",
        },
      }));
      if (!fullyRemoved) {
      ...
      assert.equal(f.reviews(), 0);
      console.log(
        `PASS: ${fullyRemoved ? "receipt-only fully removed" : "verified partial legacy residual"} resumes v4 retry/Backlog failures, leaves old evidence read-only and never resurrects after fresh Backlog completion`,
      );
    } finally {
      owner.release();
    }
