...
import {
  ...
  settle,
  repairResult,
} from "./conflict-handoff-fixture.js";
...
for (const interruptedConsume of [false, true]) {
  ...
  try {
    ...
    const { requestKey, baseSha, taskSha } = JSON.parse(
      f.comments[0].body.split("\n")[1],
    );
    const request = { requestKey, baseSha, taskSha };
    if (interruptedConsume) {
      f.setHook((event) => {
        if (event === "read:comments" && f.card.assignees.includes("bot"))
          f.setRevision(false);
      });
      await f.loop.tickNow();
      const pending = JSON.parse(
        readFileSync(
          join(f.repo, ".pi/board-agent/repair", `${requestKey}.json`),
          "utf8",
        ),
      );
      assert.equal(pending.step, "consume");
    ...
    try {
      ...
      assert.equal(f.comments[0].id, commentId);
      assert.match(f.comments[0].body, /"phase":"consumed"/);
      for (const event of [
        "request-comment",
        "reopen",
        "queue-comment",
      ...
      assert.equal(
        ...
        advanced,
        "repair never merges or closes itself",
      );
      const result = git(f.record.path, "rev-parse", "HEAD");
      git(f.repo, "merge-base", "--is-ancestor", baseSha, result);
      assert.throws(
        () => git(f.repo, "merge-base", "--is-ancestor", advanced, result),
        "repair uses the designated base, not latest main",
      );
      console.log(
        `PASS: ${interruptedConsume ? "unattempted consume" : "queued"} repair survives main fast-forward/restart with the same Issue/comment/request/SHAs/worktree and one run, then waits in open Review`,
      );
    } finally {
...
for (const event of ["request-comment", "status:Ready"]) {
  ...
  let advanced = "";
  f.setHook((point) => {
    if (!advanced && point === `after:${event}`) advanced = advanceBase(f);
  });
  try {
    await f.loop.tickNow();
    await f.loop.stop();
    ...
    try {
      ...
      assert.ok(
        f.notices.some((s) =>
          s.includes("Conflict base advanced before handoff"),
        ),
      );
      console.log(
        `PASS: main advance at closed ${event} still blocks the original handoff before reopen across restart`,
      );
    } finally {
      await next.loop.stop();
    }
...
for (const mode of [
  ...
  try {
    ...
    advanceBase(f);
    if (mode === "base-rewritten")
      git(f.origin, "update-ref", "refs/heads/main", f.originalBase);
    if (mode === "local-task")
      git(f.record.path, "commit", "--allow-empty", "-m", "human task edit");
    if (mode === "remote-task") {
      const moved = git(
        f.repo,
        "commit-tree",
    ...
    const events = [...f.events],
      comments = structuredClone(f.comments),
      next = f.make();
    try {
      await next.loop.tickNow();
      await next.loop.tickNow();
      ...
      if (["base-rewritten", "local-task", "remote-task"].includes(mode)) {
        assert.equal(
          ...
          "existing unsafe-worktree quarantine stays authoritative",
        );
        assert.equal(f.events.includes("consume-comment"), false);
        assert.deepEqual(
          f.comments[0],
          comments[0],
          "quarantine preserves the original repair marker",
        );
      } else {
        assert.deepEqual(f.events, events);
        assert.deepEqual(f.comments, comments);
      }
      assert.deepEqual(readdirSync(dir), names);
      assert.deepEqual(
        JSON.parse(readFileSync(join(dir, names[0]), "utf8")).request,
        original.request,
      );
      assert.ok(existsSync(f.record.path));
      console.log(
        `PASS: open queued ${mode} drift still blocks after main advance/restart, retaining the original request and worktree`,
      );
    } finally {
      await next.loop.stop();
    }
