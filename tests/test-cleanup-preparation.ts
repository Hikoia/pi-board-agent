...
try {
  for (const boundary of ["ignored clean", "remote deletion"] as const) {
    for (const race of ["dirty", "lock", "local", "base"] as const) {
      ...
      faults.afterGit = (a) => {
        if (
          boundary === "ignored clean"
            ? a[0] !== "clean"
            : a[0] !== "push" ||
              !a.some((s) => s.startsWith(":refs/heads/task/"))
        )
          return;
        faults.afterGit = undefined;
        result = f.tip();
        if (race === "dirty")
          writeFileSync(file, "new uncommitted user code\n");
        if (race === "lock")
          writeFileSync(join(f.admin, "index.lock"), "new operation");
        if (race === "local") {
          const newer = git(
            f.repo,
            "commit-tree",
      ...
      try {
        ...
        console.log(
          `PASS: ${race} race during ${boundary} await blocks removal; cleanup-only retry preserves code/locks/refs/approval and never remerges`,
        );
      } finally {
        faults.afterGit = undefined;
        await f.loop.stop();
