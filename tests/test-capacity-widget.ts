...
const card: Card = { itemId: "ITEM_1", number: 1, contentType: "Issue", type: "Task", status: "Needs Design", plan: "demo", title: "Task", body: "Acceptance", closed: false, assignees: [], repoOwner: "owner", repoName: "repo" };
...
const stubs: Record<string, string> = {
  "./gh.js": shim("gh.ts", ["getProjectMetadata", "listCards", "getCard", "listIssueComments", "tryClaim", "release"]),
...
WorkflowAgent.prototype.run = (async () => { enter(); await held; throw new Error("offline model finished"); }) as typeof original;
...
try {
  ...
  try {
    ...
    assert.ok(widget?.includes("  Task #1 [design]"));
