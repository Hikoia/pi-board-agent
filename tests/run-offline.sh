...
if (tasks[0].planBranch === cfg.branches.base) console.log("PASS: new task branches use the direct-merge base");
else console.log("FAIL: new task branch still uses a plan branch baseline");
...
if (store.ensure({ ...task, planBranch: "main" }, "demo").path === worktree.path) console.log("PASS: legacy plan-based worktree resumes after direct-merge upgrade");
else console.log("FAIL: legacy plan-based worktree did not resume");
...
try { git(repo, "show", "origin/main:feature.txt"); } catch { visibleBeforeClose = false; }
...
store.mergeAndRemove(worktree, "squash", 42, "Validated change", "main");
git(repo, "fetch", "origin", "main");
if (git(repo, "show", "origin/main:feature.txt") === "validated") console.log("PASS: manual-close finalization merges task into main");
else console.log("FAIL: task was not merged into main");
let visibleOnPlan = true;
...
else console.log("FAIL: stale local main creates a false task delta");
store.remove(cleanWorktree);
