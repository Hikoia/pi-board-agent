...
function harness(f: Awaited<ReturnType<typeof fixture>>) {
  ...
  const noWrite = async (): Promise<never> => {
    assert.fail("finalization must not claim, comment or launch work");
  };
    ...
    const executor = new ManagedTicketExecutor({
      ...
      callback: (message, level = "info") => {
        assert.equal(
          level,
          "info",
          "executor must return blockers, not notify them",
        );
        successes.push(message);
      },
      createManager: () => {
        assert.fail("unexpected model manager");
      },
    });
...
try {
  ...
  try {
    ...
    for (let tick = 1; tick <= 3; tick++) {
      ...
      assert.equal(
        calls.filter((args) => args[0] === "merge-tree").length,
        tick,
        "EVERY tick retries the real conflicting merge check",
      );
      assert.equal(
        calls.filter((args) => args[0] === "ls-remote").length,
        tick,
        "remote refs are refreshed, never cached as authority",
      );
      assert.equal(
        h.reads.length,
        tick * 2,
        "reconcile AND finalization read fresh approval each tick",
      );
    }
    assert.equal(
      h.warnings.length,
      1,
      "three real conflict checks must emit just one loop warning",
    );
    ...
    for (const value of [baseSha, taskSha, f.task.taskBranch])
      assert.ok(h.warnings[0].includes(value));
    assert.deepEqual(h.successes, []);
    assert.equal(h.state.tickCount, 3);
    assert.equal(h.state.wavesLaunched, 0);
    ...
    assert.ok(
      !calls.some(
        (args) =>
          ["commit-tree", "push", "update-ref"].includes(args[0]) ||
          (args[0] === "worktree" && args[1] !== "list"),
      ),
    );
    console.log(
      "PASS: three real loop/executor/local-branch conflicts recheck three times but warn once, preserving approval, refs, worktree and v3 record",
    );

    const tick = async (merges: number, warnings: number, label: string) => {
      const beforeCalls = calls.length,
        beforeWarnings = h.warnings.length;
      await h.loop.tickNow();
      assert.equal(
        calls.slice(beforeCalls).filter((args) => args[0] === "merge-tree")
          .length,
        merges,
        `${label}: retry checks`,
      );
      assert.equal(
        h.warnings.length - beforeWarnings,
        warnings,
        `${label}: new warnings`,
      );
      assert.deepEqual(h.successes, []);
    ...
    faults.beforeGit = (args) => {
      if (args[0] === "merge-tree") throw new Error(diagnostic);
    };
    await tick(1, 1, "same reason but conflict -> blocked");
    assert.equal(
      h.warnings.at(-1),
      `Finalization blocked for "${h.card.title}": ${diagnostic}`,
    );
    ...
    console.log(
      "PASS: category alone distinguishes an ordinary process failure from a conflict with the identical diagnostic",
    );

    let reason = "fetch temporarily unavailable";
    faults.beforeGit = (args) => {
      if (args[0] === "fetch") throw new Error(reason);
    };
    ...
    await tick(
      0,
      1,
      "return to an earlier reason is a new incident, not an ever-seen set",
    );
    faults.beforeGit = undefined;
    await tick(1, 1, "fetch recovery reveals conflict again");
    console.log(
      "PASS: unchanged ordinary blockers warn once; changed or returning reasons warn again without suppressing retries",
    );
    ...
    git(
      ...
      "--allow-empty",
      "-m",
      "new base SHA, same conflicting content",
    );
    git(f.repo, "push", "origin", "main");
    const nextBase = f.tip();
    await tick(1, 1, "changed base SHA");
    assert.ok(h.warnings.at(-1)!.includes(nextBase));
    assert.ok(h.warnings.at(-1)!.includes(taskSha));
    ...
    git(
      ...
      "--allow-empty",
      "-m",
      "new task SHA, same conflicting content",
    );
    git(f.record.path, "push", "origin", f.task.taskBranch);
    const nextTask = git(f.record.path, "rev-parse", "HEAD");
    await tick(1, 1, "changed task SHA");
    assert.ok(h.warnings.at(-1)!.includes(nextBase));
    assert.ok(h.warnings.at(-1)!.includes(nextTask));
    await tick(1, 0, "same new task SHA");
    console.log(
      "PASS: advancing either real base or task SHA reports the new conflict once",
    );
    ...
    await tick(
      1,
      0,
      "other ticket does not replace original ticket fingerprint",
    );
    await h.loop.stop();
    ...
    console.log(
      "PASS: identical blockers on different tickets notify independently; a fresh loop warns again with review.enabled unchanged",
    );
    ...
    const read = h.board.getCard;
    let reads = 0;
    h.board.getCard = async (id) => {
      if (++reads === 2) h.card.closed = false;
      return read(id);
    };
    await tick(0, 0, "fresh reopened approval skips finalization");
    h.board.getCard = read;
    h.card.closed = true;
    await tick(1, 1, "same failure after a skipped outcome");
    await tick(1, 0, "same post-skip failure");
    console.log(
      "PASS: skipped fresh approval clears the previous blocker so the same later failure warns anew",
    );

    const eligible = structuredClone(h.card);
    const ineligible: Array<[string, Partial<Card>]> = [
      ["open issue", { closed: false }],
      ["different lane", { status: h.cfg.columns.needs_human }],
      ["pull request", { contentType: "PullRequest" }],
      ["foreign repository", { repoOwner: "other" }],
      ["missing issue identity", { number: undefined }],
    ];
    for (const [label, patch] of ineligible) {
      ...
      assert.ok(
        !calls.slice(beforeCalls).some((args) => args[0] === "ls-remote"),
        "no task-ref query for an ineligible board",
      );
    ...
    console.log(
      "PASS: irrelevant entries are pruned for open, changed-lane/type/repository/identity and missing tickets, including an empty candidate list",
    );
    ...
    await tick(
      1,
      0,
      "remote-only branch restores the exact source and retries the same conflict",
    );
    assert.equal(f.store.localBranchSha(f.task.taskBranch), nextTask);
    await tick(1, 0, "same returning branch failure");
    console.log(
      "PASS: a remote-only conflict restores its ref safely and keeps the same blocker fingerprint",
    );
  } finally {
    faults.beforeGit = undefined;
    await h.loop.stop();
...

  {
    ...
    try {
      ...
      h.card.status = h.cfg.columns.done;
      faults.beforeGit = failFetch;
      await h.loop.tickNow();
      await h.loop.tickNow();
      assert.equal(
        h.warnings.length,
        2,
        "same failure after successful finalization must notify again",
      );
      assert.equal(h.warnings[0], h.warnings[1]);
      assert.equal(h.successes.length, 1);
      console.log(
        "PASS: successful real integration/cleanup clears the blocker before an identical later failure",
      );
    } finally {
      faults.beforeGit = undefined;
      await h.loop.stop();
...

  {
    const f = await fixture(),
      h = harness(f);
    let attempts = 0,
      reason = "cleanup record is locked";
    faults.beforeFs = (operation, path) => {
      if (operation === "unlink" && path === f.recordFile) {
        attempts++;
    ...
    try {
      ...
      for (let tick = 1; tick <= 3; tick++) {
        ...
        assert.equal(
          attempts,
          tick,
          "receipt retries EVERY tick even after local-ref deletion",
        );
        assert.equal(
          h.warnings.length,
          1,
          "repeated cleanup blocker warns once",
        );
      ...
      assert.equal(
        f.tip(),
        integrated,
        "cleanup retries must not integrate again",
      );
      ...
      assert.equal(h.warnings.length, 2);
      console.log(
        "PASS: no-ref cleanup receipts retry on every tick, deduplicate blockers, report changed reasons and finish without duplicate integration",
      );
    } finally {
      faults.beforeFs = undefined;
      await h.loop.stop();
  ...
  }
  {
    ...
    const base = f.tip(),
      dates = [process.env.GIT_AUTHOR_DATE, process.env.GIT_COMMITTER_DATE];
    let commits = 0;
    faults.beforeGit = (args) => {
      if (args[0] === "commit-tree") {
        process.env.GIT_AUTHOR_DATE =
          process.env.GIT_COMMITTER_DATE = `2000-01-01T00:00:0${++commits}Z`;
      }
    };
    try {
      for (let i = 0; i < 3; i++) await h.loop.tickNow();
      assert.equal(
        commits,
        3,
        "each attempt creates a deliberately different temporary source commit",
      );
      assert.equal(
        h.warnings.length,
        1,
        "same original sources/base must deduplicate despite changing temporary SHAs",
      );
      for (const sha of [base, local, remote])
        assert.ok(h.warnings[0].includes(sha));
      assert.deepEqual(h.successes, []);
      ...
      console.log(
        "PASS: divergent sources conflicting with base deduplicate on original SHAs, not changing temporary commit identities",
      );
    } finally {
      faults.beforeGit = undefined;
      for (const [i, key] of [
        ...
        if (dates[i] === undefined) delete process.env[key];
        else process.env[key] = dates[i];
      }
      await h.loop.stop();
    }
  }
