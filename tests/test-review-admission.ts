...
try {
  for (const change of ['revision', 'latched', 'unobserved-revision', 'local-revision', 'stop', 'unchanged']) {
    const f = fixture(); f.cfg.safety.require_clean_worktree = false;
    ...
    const entered = deferred(), finish = deferred(), latched = deferred();
    ...
    let revision = true, localRevision = true, calls = 0, releases = 0, done = false;
    ...
    try {
      ...
      if (change === 'revision' || change === 'latched') {
        revision = false; await latched.promise;
        assert.equal(loop.isAdmittingNewWork(), false);
        if (change === 'latched') revision = true;
      }
      if (change === 'unobserved-revision') revision = false;
      if (change === 'local-revision') localRevision = false;
