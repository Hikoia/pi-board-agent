...
import { runProcess, runProcessSync, type ProcessCommand, type ProcessOptions } from "../src/process-runner.js";
...
export const faults: { beforeGit?: (args: string[], options: ProcessOptions) => void | Promise<void>; beforeFs?: (operation: string, path: string) => void | Promise<void>; afterFs?: (operation: string, path: string) => void | Promise<void> } = {};
const globals = globalThis as any;
globals.__cleanupFs = async (operation: "unlink" | "link" | "open" | "readdir" | "readlink", path: string, ...args: any[]) => {
...
globals.__cleanupGit = (mode: "sync" | "async", command: ProcessCommand, args: string[], options: ProcessOptions = {}) => {
  calls.push(args);
  assert.ok(!(args[0] === "worktree" && ["prune", "unlock"].includes(args[1])), "never prune/unlock");
  assert.ok(!(args[0] === "worktree" && args[1] === "remove" && args.includes("--force")), "normal remove only");
  if (mode === "sync") return runProcessSync(command, args, options);
  return (async () => { await faults.beforeGit?.(args, options); return runProcess(command, args, options); })();
};
...
const hooks = registerHooks({ resolve(specifier, context, next) {
  if (context.parentURL === worktreeUrl && specifier === "./process-runner.js") return {
    url: `data:text/javascript,${encodeURIComponent(`export * from ${JSON.stringify(runnerUrl)};
      ...
      export const runProcessSync = (...args) => globalThis.__cleanupGit('sync', ...args);`)}`, shortCircuit: true,
  };
  if ([worktreeUrl, new URL("../src/cleanup-snapshot.ts", import.meta.url).href].includes(context.parentURL!) && specifier === "node:fs/promises") return {
    url: `data:text/javascript,${encodeURIComponent(`export * from 'node:fs/promises';
      ...
      export const readlink = (...args) => globalThis.__cleanupFs('readlink', ...args);`)}`, shortCircuit: true,
  };
...
export async function fixture(lockfiles = false) {
    ...
    const task: BuilderTask = { itemId: `ITEM_${sequence}`, issueNumber: sequence, taskKey: `T${sequence}`, title: "Accepted", body: "acceptance", taskBranch: `task/issue-${sequence}`, baseBranch: "main" };
    const store = new TicketWorktrees(repo), record = await store.ensure(task, "demo");
    writeFileSync(join(record.path, "feature.txt"), "feature\n"); git(record.path, "add", "."); git(record.path, "commit", "-m", "feature");
    ...
    const finish = (strategy: "merge" | "squash" = "merge") => new TicketWorktrees(repo).finalizeAccepted(task, strategy);
    const vanish = () => { unlinkSync(join(record.path, ".git")); rmSync(admin, { recursive: true }); };
    return { repo, origin, base, task, store, record, taskSha, admin, receipt, recordFile, tip, finish, vanish };
  }

export function legacy(f: Awaited<ReturnType<typeof fixture>>, strategy: "merge" | "squash", journal: "none" | "intent" | "result") {
    const result = git(f.repo, "commit-tree", `${f.taskSha}^{tree}`, "-p", f.base, ...(strategy === "merge" ? ["-p", f.taskSha] : []), "-m", "previously integrated");
    git(f.repo, "push", "origin", `${result}:refs/heads/main`);
    if (journal !== "none") f.store.update(f.task.itemId, (record) => ({ ...record, finalization: { targetBranch: "main", baseSha: f.base, taskSha: f.taskSha, ...(journal === "result" ? { resultSha: result } : {}) } }));
    f.vanish();
    return result;
  }

export function dispose() { faults.beforeGit = faults.beforeFs = faults.afterFs = undefined; hooks.deregister(); delete globals.__cleanupGit; delete globals.__cleanupFs; }
