...
for (const delta of [false, true]) {
  ...
  const entered = deferred(),
    finish = deferred(),
    hasTaskDelta = f.store.hasTaskDelta.bind(f.store);
  ...
  try {
    await Promise.race([
      entered.promise,
      recovering.then(() => {
        throw new Error("recovery settled before delta observation");
      }),
    ]);
    ...
    assert.deepEqual(
      ...
      launchRecord,
      "pending fetch preserves exact crash evidence",
    );
    finish.resolve();
    ...
    assert.equal(
      f.card.status,
      delta ? cfg.columns.needs_human : cfg.columns.ready,
    );
    assert.ok(existsSync(record.path));
    assert.equal(f.store.read(f.card.itemId)?.launchingAt, undefined);
    console.log(
      `PASS: crash recovery awaits hasTaskDelta=${delta} before returning Ready or quarantining local work`,
    );
  } finally {
    finish.resolve();
    await recovering;
...

{
  ...
  try {
    ...
    assert.deepEqual(f.writes, [cfg.columns.backlog]);
    console.log(
      "PASS: loop/executor await finalization and stop drains it without aborting destructive cleanup",
    );
  } finally {
    finish.resolve();
    await tick;
