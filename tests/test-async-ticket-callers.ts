...
git(root, "init", "--bare", origin); git(repo, "init", "-b", "main");
...
function fixture() {
  ...
  const board: TicketBoardAdapter = {
    ...
    release: async () => { writes.push("release"); card.assignees = []; },
  ...
  const executor = new ManagedTicketExecutor({ cwd: repo, cfg, worktrees: store, board, botLogin: "bot", repoOwner: "owner", repoName: "repo", callback: (s) => notices.push(s), createManager: () => ({
    start: (_source, args) => { starts++; run = { args, runId: `run-${number}`, status: "running" } as PersistedRunState; return run.runId; },
    list: () => run ? [run] : [], resume: async () => false,
...
for (const change of ["unchanged", "revision", "human", "contract", "stop"]) {
  ...
  f.store.ensure = (...args) => preparation = (async () => {
  ...
  try {
    ...
    if (change === "stop") {
      ...
      assert.equal(stopped, false, "stop waits for the in-flight preparation and cleanup");
    }
    finish.resolve();
    await tick; await stopping;
    assert.equal(f.starts(), change === "unchanged" ? 1 : 0);
    const record = f.store.read(f.card.itemId)!;
    assert.ok(existsSync(record.path), "no destructive worktree cleanup on a skipped launch");
...

{
  ...
  let stopped = false;
  try {
    await Promise.race([entered.promise, tick.then(() => { throw new Error("finalization not reached"); })]);
    const stopping = f.loop.stop().then(() => { stopped = true; });
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(stopped, false, "stop may not detach destructive finalization or release its owner early");
