...
export async function fixture(legacy = false, autoMerge = true) {
  ...
  const card: Card = {
    ...
    title: f.task.taskKey + " " + f.task.title,
    body: f.task.body,
    contentType: "Issue",
    type: "Task",
    plan: "demo",
  ...
  const board: TicketBoardAdapter = {
    ...
    claim: async () => {
      if (failClaim) throw new Error("offline claim failure");
      card.assignees = ["bot"];
      return true;
    },
    release: async () => {
    ...
    setStatus: async (_id, status) => {
      ...
      if (status === cfg.columns.backlog) {
        assert.ok(
          ...
          "record survives until Project Backlog",
        );
      ...
      card.status = status;
      if (status === cfg.columns.backlog && loseDone)
  ...
  const loop = new BoardLoop(
    {
      ...
      review: async () => {
        reviews++;
        throw new Error("No review during integrate or cleanup");
      },
      boardOps: {
        claim: board.claim,
        refresh: () => board.getCard(card.itemId),
  ...
  const noNewEvidence = () => {
    ...
    assert.equal(
      existsSync(join(f.repo, ".pi", "board-agent", "repair")),
      false,
    );
  };
