...
for (const edge of ["before", "after"] as const) {
  ...
  try {
    ...
    try {
      ...
      assert.equal(f.events.filter((e) => e === "request-comment").length, 1, "never replay a nonidempotent request create, even after ambiguous absence");
      assert.equal(f.comments.filter((c) => c.body.startsWith("<!-- board-agent-conflict-repair:")).length, target === "request-comment" && edge === "before" ? 0 : 1);
      if (["request-comment", "status:Ready", "reopen", "queue-comment", "consume-comment"].includes(target) && edge === "before") {
        assert.equal(f.calls(), 0, "unconfirmed attempted transition blocks, never ordinary Ready work");
        assert.equal(f.events.filter((e) => e === target).length, 1, "unconfirmed write not blindly replayed");
      } else if (target === "consume-comment" || target === "status:In Progress") {
...
for (const target of ["read:comments", "read:card"]) {
  ...
  try {
    ...
    assert.equal(f.card.closed, false); assert.equal(f.card.status, f.cfg.columns.ready); assert.equal(f.calls(), 0);
    ...
    try {
      ...
      console.log(`PASS: ${target} ambiguity at open Ready blocks ordinary work, then confirms the same handoff after restart`);
    } finally { await next.loop.stop(); }
  } finally { await f.loop.stop(); }
}
