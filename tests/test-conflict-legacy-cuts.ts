...
try {
  for (const cut of [
    ...
    if (cut === "before-publish")
      ...
      faults[cut === "before-archive" ? "beforeFs" : "afterFs"] = (
        op,
        path,
      ) => {
        if (op === "link" && path.includes("legacy-v3")) fail();
      };
    try {
      ...
      console.log(
        `PASS: ${cut} pre-result migration interruption preserves exact raw intent, then reentrant conversion/finalization needs no old repair-intent clear protocol`,
      );
    } finally {
      faults.beforeFs = faults.afterFs = faults.beforeSyncFs = undefined;
      owner.release();
